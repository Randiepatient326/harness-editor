...
클로드 코드로 에이전트와 스킬은 쉽게 만들 수 있는데, 정교하게 깎으려니 — frontmatter 다듬기, description 보완, 파일 간 표 비교 — 일반 에디터로는 답답합니다. 그 한 가지에 집중한 작은 도구입니다.
...
- Description 적극성 점수·트리거 키워드 분석 ([revfactory/harness](https://github.com/revfactory/harness) 메타스킬 권장 패턴)
...
|----|------|
| `⌘S` / `Ctrl+S` | 현재 파일 저장 |
| `⌘⇧S` / `Ctrl+Shift+S` | 변경된 모든 파일 저장 |
| `⌘P` / `Ctrl+P` | 모든 파일 가로질러 검색 |
| `⌘⇧H` / `Ctrl+Shift+H` | 일괄 치환 |
| `⌘B` / `Ctrl+B` | 사이드바 토글 |
| `⌘\` / `Ctrl+\` | 프리뷰 토글 |
| `Esc` | 모달 닫기 |
...
레포에서 `harness-editor.html` 파일을 다운로드 후 더블클릭하면 됩니다. 외부 자원은 CDN뿐, 추가 설치 없음.
